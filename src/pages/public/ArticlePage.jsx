...
export default function ArticlePage() {
  ...
  if (!article) return (
    <div className="page-container text-center py-24">
      <p className="text-slate-400 text-xl">Artikel tidak ditemukan atau belum dipublikasikan.</p>
    ...
    <div className="page-container max-w-4xl">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-8 flex-wrap">
        ...
        {topic && (
          <>
            <ChevronRight size={14} />
            <Link to={`/topik/${topic.slug}`} className="hover:text-slate-300 transition-colors">{topic.name}</Link>
          </>
        )}
        <ChevronRight size={14} />
      ...
      {/* Article header */}
      <h1 className="text-4xl font-extrabold text-white leading-tight mb-4">{article.title}</h1>
      ...
      <article className="prose prose-invert prose-slate max-w-none
        ...
        prose-code:text-accent-300 prose-code:bg-dark-700 prose-code:px-1 prose-code:py-0.5 prose-code:rounded
        prose-pre:bg-dark-800 prose-pre:border prose-pre:border-slate-700
        prose-blockquote:border-brand-500 prose-blockquote:bg-brand-500/5 prose-blockquote:text-slate-300
